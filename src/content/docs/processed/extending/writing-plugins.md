...
| -------- | ------- | ------- |
|       | Router | [HTTP server plugins](/docs/extending/http-server-plugins/) modify the HTTP request and response between the end-user and Pucora (the server)    |
|     | Proxy | [Request/Response Modifier plugins](/docs/extending/plugin-modifiers/) modify data (such as headers, body, status code...) |
|       | Transport | [HTTP client plugins](/docs/extending/http-client-plugins/) modify the request and response between Pucora and the upstream services (internal HTTP client)    |
...
Before compiling the plugin, you must ensure the libraries you use in your code are compatible with Pucora. To do so, execute the command [``](/docs/extending/check-plugin/) that analyzes your `go.sum` file and warns you about incompatibilities.
...
## Compile the plugin
> **Do not compile locally, use the builder**
>
...
|--------------|---------------------------------------------------------------------|-----------------------------------------------------------------------------------|
| [AMD64](/docs/extending/writing-plugins/#compile-plugins-for-amd64)        | `:` | `:-linux-generic` |
| [ARM64](/docs/extending/writing-plugins/#compile-plugins-for-arm64)        | `:` with cross-compile instructions | `:-linux-generic` with cross-compile instructions |
...
```bash
docker run -it -v "$PWD:/app" -w /app : go build -buildmode=plugin -o yourplugin.so .
```


The command will generate a `yourplugin.so` file (name it as you please) that you can now copy into a `pucora/pucora-ce:latest:` Docker image, and load it as described in [injecting plugins](/docs/extending/injecting-plugins/). **Never use a tag that mismatches the builder and Pucora**. If you want to load the plugin in a Pucora version `x.y.x` make sure to build it on a builder `x.y.z`. Using `.so` files that were compiled in a builder with a different version, will mostly fail.
...
```bash
docker run -it -v "$PWD:/app" -w /app :-linux-generic go build -buildmode=plugin -o yourplugin.so .
```
