...
> **You must build the tag `:watch` on the Community Edition**
>
...
cd pucora-watch
docker build -t pucora/pucora-ce:latest:watch .
```
...
After the build completes, you will be able to use `pucora/pucora-ce:latest:watch` normally.
...
```bash
docker run --rm -v "$PWD:/etc/pucora" pucora/pucora-ce:latest:watch
```
...
docker run --rm -it -v "$PWD:/etc/pucora" \
    ...
    pucora/pucora-ce:latest:watch run -c pucora.tmpl
```
